...
def install(section, event, loggedInUser, appText, plistText, appPath):
    ...
    plistbuddy = (
        'sudo -u %s /usr/libexec/PlistBuddy -c "Set :%sInstallStatus %s" %s'
        % (loggedInUser, plistText, statusstart, filePath)
    )
    sub_command = [plistbuddy]
    ssUser = subprocess.Popen(
        sub_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    )
    ...
    ssUser = subprocess.Popen(
        sub_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    ...
    plistbuddy = (
        'sudo -u %s /usr/libexec/PlistBuddy -c "Set :%sInstallStatus %s" %s'
        % (loggedInUser, plistText, statusstop, filePath)
    )
    sub_command = [plistbuddy]
    ssUser = subprocess.Popen(
        sub_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
...
if bundlename in bundles:
    ...
    install_event_list = {
        "AppText1": [
            ...
            "AppPlistUpdateName1",
            "AppJPSEventTrigger1",
            "AppInstallVerifyPath1",
        ],
        "AppText2": [
            "AppInstallOrder2",
            "AppPlistUpdateName2",
            "AppJPSEventTrigger2",
            "AppInstallVerifyPath2",
        ],
    }
    install_event_list = OrderedDict(
        sorted(install_event_list.items(), key=lambda t: t[1])
    )
